...
function GradeSummaryTableFooter({ intl }) {
  ...
  const statusColor = isPassing ? 'bg-success-100 border-success-500 text-success' : 'bg-warning-100 border-warning-500 text-warning';
  const totalGrade = (visiblePercent * 100).toFixed(0);

  return (
    <DataTable.TableFooter className={`border rounded ${statusColor}`}>
